...
public class FPSRigidBodyWalker : MonoBehaviour {
    ...
    public static FPSRigidBodyWalker instance;
    //	[HideInInspector]
...
//		startingPos = myTransform.position;
				...
				if(allowSprinting){
					...
					if((Mathf.Abs(inputY) > 0.0f && forwardSprintOnly) || (!forwardSprintOnly && (Mathf.Abs(inputX) > 0.0f) || Mathf.Abs(inputY) > 0.0f )){
						if(InputComponent.sprintHold){
							if(!sprintStartState){
                                //TimerScript.Instance.decrementImgVal = (3 / TimerScript.Instance.timeLeft);
							...
							if(!sprintEndState){
                                ...
                                TimerScript.Instance.decrementImgVal = (1 / TimerScript.Instance.timeLeft);
	...
	void OnCollisionEnter (Collision col){
        ...
        {
            for(int i = 0;i<3;i++)
            TimerScript.Instance.DecreaseBattery();
            FPSPlayerComponent.ApplyDamage(2);
        }
